...
void position(int c[], double *markX, double *markO) {
	double X = 0.0, O = 0.0;
...
static double calculate(State s, int pos) {
	...
	for (i = 1; i <= 9; i++) {
		if (type[i] == 2) {
			if (indegreeX[i] != indegreeO[i]) {
				hcost = hcost + 35.0 * score(0) * indegreeX[i] + 
					35.0 * score(1) * indegreeO[i];
			}
		} else if (type[i] == 1) {
			...
			double markX = 0.0, markO = 0.0;
			position(s.board[i], &markX, &markO);
			hcost = hcost + (markX * indegreeX[i] + markO * indegreeO[i]) * 0.4;
		}
	}
...
int agent_second_move( int board_num, int prev_move )
{
  ...
  Play next = Minimax(current, player,0, -4096.0, 4096.0);
  this_move = next.move;
  move[m] = this_move;
