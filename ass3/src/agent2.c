...
static double estimate(int c[], int currentplayer) {
	...
	double cost = 0.0;
	int win = 0, lost = 0;
	// 5 points for the centre of the board;
	cost = cost + 5.0 * score(c[5]);
	// 2 points for the corner of the board;
	cost = cost + 2.0 * (score(c[1]) + score(c[3]) + score(c[7]) + score(c[9]));
	// getting 7 points for each 2 consecutive non-blocked occupation
	...
	cost = cost + win * 1.0 * 7.0 * score(currentplayer);
	// lost 4.5 points for each 2 consecutive non-blocked occupation
	// equals the ! current player
	cost = cost + lost * 1.0 * 4.5 * score(!currentplayer);
	return cost;
} 

double calculate(State s, int currentplayer, int pos) {
	int i;
	s.previous = pos;
	...
	double relevant = estimate(s.board[pos], !currentplayer);
	double irrelevant = 0.0;
	for (i = 1; i <= 9; i++) {
		if (pos != i) {
			// srand(time(NULL));
			irrelevant = irrelevant + estimate(s.board[i], currentplayer);
		}
	}
	return relevant * 0.7 + irrelevant * 0.3;
}
// if the game is over return 0 or -200 or 200 otherwise return 2
static int gameover(State s) {
...
int agent_next_move( int prev_move )
{
  ...
  if (next.heuristic == score(!player) * 200) {
  	printf("random move\n");
  	do {
    	this_move = 1 + random()% 9;
  	} while( board[prev_move][this_move] != EMPTY );
