...
the reason we design 0/1/2/4/8 
is we can bitwise or them*/
...
double calculate(State s, int currentplayer) {
	...
	for (i = 1; i <= 9; i++) {
		if (i != s.nextboard) {
			mark += ((X_WIN & s.status[i]) != 0) * 30.0 * score(0);
			mark += ((X_ADVANTAGE & s.status[i]) != 0) * 7.0 * score(0);
			mark += ((X_POS & s.status[i]) != 0) * 2.0 * score(0);
			mark += ((O_WIN & s.status[i]) != 0) * 30.0 * score(1);
			mark += ((O_ADVANTAGE & s.status[i]) != 0) * 7.0 * score(1);
			mark += ((O_POS & s.status[i]) != 0) * 2.0 * score(1);
		} else {
		
			if (currentplayer == 0) {
				mark += ((X_WIN & s.status[i]) != 0) * 200.0 * score(0);
				mark += ((O_WIN & s.status[i]) != 0) * 30.0 * score(1);
			} else {
				mark += ((X_WIN & s.status[i]) != 0) * 30.0 * score(0);
				mark += ((O_WIN & s.status[i]) != 0) * 200.0 * score(1);
			}
...
static int searchdepth(int moves) {
	...
	if (moves <= 30) {
		if (simple == 1) return 13; // current state is very simple
		if (moves <= 24) return 11; // 18 - 24 moves
		return 13;
	}
	if (moves <= 54) return 15; // 31 - 54 moves
	return 17; // 55 - 81 moves
}
